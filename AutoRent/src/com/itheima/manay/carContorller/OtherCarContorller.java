...
public class OtherCarContorller {
    ...
    public void findAllCar() {
        if(allCar.size() == 0){
            ...
            System.out.println("编号\t\t品牌\t\t租金（每日）\t\t吨位");
            for (int i = 0; i < allCar.size(); i++) {
                Car car = allCar.get(i);
                if(car.getBrand().equals("卡车")){
                    Truck t = (Truck) car;
                    System.out.println(t.getId() + "\t\t"+ t.getBrand() + "\t\t"+ t.getMoney()+"\t\t"+t.getWeight());
                }else {
    ...
    public  void addCar() {
        ...
        String brand = sc.next();
        if ("卡车".equals(brand)) {
            System.out.println("请输入卡车租金");
            int money = sc.nextInt();
            System.out.println("请输入卡车吨位：");
            int weight = sc.nextInt();
            Car car = new Truck(id, brand, money, weight);
